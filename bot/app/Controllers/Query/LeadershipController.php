...
class LeadershipController extends QueryController
{
    ...
    public function home($index)
    {
        ...
        foreach ($res as $key => $m) {
            if ($key == 10)
                continue;
            $text .=  "*#" . $m->getRank() . "*\n ➥ [ " . $m->getName() . " " . $m->getParsedSeason() . "](https://t.me/myanimetvbetabot?start=movieID_" . $m->getId() . ") *" . $m->getScore() . "⭐️ ~ " . $m->getTotalVotes() . " voti*\n\n";
        }
        ...
        $this->query->message->edit($text, $menu, "Markdown", false);
    }
}
